...
export const savePartState = async (selectedPart, setSavingPart, setSet, setModalVisible, setGlobalError) => {
    ...
    setSavingPart(true);
    try {
        const response = await fetch(`${Config.API_BASE_URL}/api/user-set-part/${selectedPart.userSetPartId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                missingQuantity: selectedPart.missingQuantity,
                damagedQuantity: selectedPart.damagedQuantity,
                discolouredQuantity: selectedPart.discolouredQuantity,
            }),
        });
        const data = await response.json();
        setSet(prev => ({
            ...
            setParts: prev.setParts.map(p =>
                p.userSetPartId === selectedPart.userSetPartId ? { ...p, ...data } : p
            ),
        }));
        setModalVisible(false);
    } catch (err) {
